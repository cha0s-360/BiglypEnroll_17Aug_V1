...
import {
  ArrowRight, Sparkles, ChevronDown, Star, Plus, Check, GraduationCap, Wallet,
  Brain, Search, FileCheck2, BookOpenCheck, Award, Landmark, Calculator, ShieldCheck,
  Gauge, LineChart, Globe, Building2, Users, Rocket, Mail, PlayCircle, Quote,
} from 'lucide-react';
...
function HomeNav() {
  ...
  const NAV = [
    ...
    { label: 'BiglypEnroll', href: '/biglypenroll', active: false },
  ];
  return (
    <Box component="header" className="sticky top-0 z-50 bg-white/80 backdrop-blur-xl border-b border-blue-100">
      <Box className="max-w-7xl mx-auto px-5 h-16 flex items-center justify-between">
        ...
        <Box component="nav" className="hidden md:flex items-center gap-1">
          {NAV.map((n) => (
            <Link key={n.label} href={n.href} data-testid={`homenav-${n.label.toLowerCase()}`}
              ...
              style={n.active ? { background: `linear-gradient(135deg, ${SKY}, ${BLUE_DEEP})` } : undefined}>
              {n.label}
            </Link>
          ))}
        </Box>
        <Box className="flex items-center gap-2.5">
          <Link href="/login">
            <Button variant="outline" className="h-9 px-5 rounded-full text-sm font-semibold border-blue-200 text-blue-700 hover:bg-blue-50">Sign in</Button>
          </Link>
          <Box component="a" href="#platform">
            <Button className="h-9 px-5 rounded-full text-sm font-semibold text-white shadow-lg shadow-blue-600/25" style={{ background: `linear-gradient(135deg, ${SKY}, ${BLUE_DEEP})` }}>
              Become a partner
            </Button>
          </Box>
        </Box>
      ...
      <Box className="relative max-w-7xl mx-auto px-5 pt-14 pb-16 grid lg:grid-cols-[1.05fr_0.95fr] gap-12 items-center">
        <motion.div initial="hidden" animate="show" variants={fade}>
          ...
          <motion.h1 custom={1} variants={fade} className="font-head mt-5 text-4xl md:text-5xl lg:text-[54px] font-black tracking-tight leading-[1.05]" style={{ color: NAVY }}>
            ...
            <span className="relative inline-block">
              <span className="relative z-10 bg-clip-text text-transparent" style={{ backgroundImage: `linear-gradient(120deg, ${SKY}, ${BLUE_DEEP})` }}>Future-Ready</span>
            </span>{' '}
            Students
          </motion.h1>
          ...
          <motion.div custom={4} variants={fade} className="mt-8 flex flex-wrap gap-3">
            <Box component="a" href="#journey">
              <Button className="h-12 px-6 rounded-full font-semibold text-sm text-white shadow-xl shadow-blue-600/25" style={{ background: `linear-gradient(135deg, ${SKY}, ${BLUE_DEEP})` }}>
                Take Free Assessment <ArrowRight className="h-4 w-4 ml-2" />
              </Button>
            </Box>
            <Box component="a" href="#platform">
              <Button variant="outline" className="h-12 px-6 rounded-full font-semibold text-sm border-blue-200 text-blue-700 hover:bg-blue-50 bg-transparent">
                <PlayCircle className="h-4 w-4 mr-2" /> Explore Platform
              </Button>
            </Box>
          </motion.div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, scale: 0.94 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.6, delay: 0.2 }} className="relative">
          ...
          <Box className="hidden sm:flex absolute -left-5 top-10 rounded-2xl bg-white shadow-xl p-3 pr-4 items-center gap-3" style={{ boxShadow: '0 20px 40px -15px rgba(37,99,235,0.3)' }}>
            ...
            <Box>
              <Typography variant="inherit" component="p" className="text-[10px] uppercase tracking-widest font-bold text-slate-400">AI Psychometrics</Typography>
              <Typography variant="inherit" component="p" className="text-sm font-head font-bold" style={{ color: NAVY }}>60+ traits mapped</Typography>
            </Box>
          </Box>
          <Box className="hidden sm:flex absolute -right-4 bottom-8 rounded-2xl bg-white shadow-xl p-3 pr-4 items-center gap-3" style={{ boxShadow: '0 20px 40px -15px rgba(37,99,235,0.3)' }}>
            ...
            <Box>
              <Typography variant="inherit" component="p" className="text-[10px] uppercase tracking-widest font-bold text-slate-400">Study abroad</Typography>
              <Typography variant="inherit" component="p" className="text-sm font-head font-bold" style={{ color: NAVY }}>42 countries</Typography>
            </Box>
          </Box>
        </motion.div>
      </Box>
...
/* =================== STATS =================== */
const STATS = [
  { icon: Globe, value: '0+', label: 'Countries', color: SKY },
  { icon: Building2, value: '0+', label: 'Partner Universities', color: '#7C3AED' },
  { icon: Award, value: '0+', label: 'Career Programs', color: '#F59E0B' },
];
function Stats() {
  return (
          ...
          {STATS.map((s) => {
            ...
            return (
              <Box key={s.label} className="flex items-center gap-4 justify-center sm:justify-start">
                ...
                <Box>
                  <Typography variant="inherit" component="p" className="font-head text-2xl font-black" style={{ color: NAVY }}>{s.value}</Typography>
                  <Typography variant="inherit" component="p" className="text-[12px] font-medium text-slate-500">{s.label}</Typography>
                </Box>
              </Box>
            );
          })}
    ...
    <Box component="section" id={id} className="py-20" style={bg ? { background: bg } : undefined}>
      <Box className="max-w-7xl mx-auto px-5">
        <Box className="text-center max-w-3xl mx-auto">
          {eyebrow && (
            <Box component="span" className="inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-[11px] font-bold uppercase tracking-widest" style={{ background: '#fff', color: BLUE_DEEP, border: '1px solid #DBEAFE' }}>
        ...
        <Box className="mt-12 grid lg:grid-cols-[300px_1fr] gap-6 items-start">
          {/* Tab list */}
            ...
            {tabs.map((t: any, i: number) => {
              ...
              return (
                <Box component="button" key={t.label} onClick={() => setActive(i)} data-testid={`tab-${id}-${i}`}
                  ...
                  style={on ? { background: `linear-gradient(135deg, ${SKY}, ${BLUE_DEEP})` } : undefined}>
          ...
          <motion.div key={active} initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}
            className="rounded-3xl bg-white border border-blue-100 p-7 md:p-9 shadow-[0_24px_60px_-30px_rgba(37,99,235,0.4)] grid md:grid-cols-[1fr_0.8fr] gap-8 items-center">
            <Box>
              <Box className="h-14 w-14 rounded-2xl flex items-center justify-center" style={{ background: TINT, color: BLUE_DEEP }}><CurIcon className="h-7 w-7" /></Box>
              <Typography variant="inherit" component="h3" className="font-head text-2xl font-black mt-5" style={{ color: NAVY }}>{cur.cardTitle}</Typography>
              {cur.desc.map((d: string, k: number) => (
                <Typography key={k} variant="inherit" component="p" className="mt-3 text-[14px] leading-relaxed text-slate-600">{d}</Typography>
              ))}
              <Box className="mt-6 flex flex-wrap gap-3">
                <Button className="h-11 px-5 rounded-full font-semibold text-sm text-white shadow-lg shadow-blue-600/20" style={{ background: `linear-gradient(135deg, ${SKY}, ${BLUE_DEEP})` }}>
                  {cur.primary} <ArrowRight className="h-4 w-4 ml-2" />
                </Button>
                {cur.secondary && (
                  <Button variant="outline" className="h-11 px-5 rounded-full font-semibold text-sm border-blue-200 text-blue-700 hover:bg-blue-50 bg-transparent">{cur.secondary}</Button>
                )}
              </Box>
            </Box>
            <Box className="rounded-2xl overflow-hidden aspect-square hidden md:block" style={{ background: `linear-gradient(160deg, ${TINT}, #fff)` }}>
              <Box component="img" src={cur.img} alt={cur.cardTitle} className="w-full h-full object-cover" />
            </Box>
          </motion.div>
        </Box>
        ...
        {footer && (
          <Box className="mt-10 text-center">
            ...
            <Box className="mt-4">
              <Button className="h-11 px-7 rounded-full font-semibold text-sm text-white shadow-lg shadow-blue-600/20" style={{ background: `linear-gradient(135deg, ${SKY}, ${BLUE_DEEP})` }}>{footer.cta}</Button>
            </Box>
          </Box>
        )}
      </Box>
...
const ENGINES = [
  {
    icon: GraduationCap, tag: 'For Institutions', href: '/biglypenroll', color: BLUE_DEEP,
    title: 'BiglypEnroll',
    tagline: 'The all-in-one OS for modern institutions.',
    desc: 'Run admissions and fee collection from a single console — bridging career readiness and payments to serve 6,500+ institutions and 50 Lakh+ students.',
    points: ['Admissions & enrolment OS', 'Career + fee-collection engines', 'ERP integration or white-labeled portal', 'ISO 27001 · DPDP · RBI-regulated'],
  },
  {
    icon: Brain, tag: 'For Students', href: '/career-hub', color: '#7C3AED',
    title: 'Biglyp Career Hub',
    tagline: 'Discover the right career. Then the right university.',
    ...
    points: ['4-D psychometrics: Aptitude · Interest · EQ · Personality', '2,50,000+ courses across 42 countries', 'Personalised career & university matches', 'Counsellor dashboards & reports'],
  },
  {
    icon: Wallet, tag: 'For Parents', href: '/fee-collection', color: '#0EA5E9',
    title: 'Biglyp Fee Collection',
    tagline: 'Fees upfront. EMIs for parents. Reconciled live.',
    desc: "India's most advanced fee payment platform for schools, colleges and skilling institutes — 8+ payment rails, 0% EMIs and live analytics in one place.",
    points: ['8+ payment rails', '0% EMIs for parents · 100% upfront to schools', 'Automated reconciliation & live analytics', 'RBI-regulated NBFC partners'],
  },
];
function PlatformEngines() {
  return (
    <Box component="section" id="platform" className="py-20 bg-white">
      <Box className="max-w-7xl mx-auto px-5">
        <Box className="text-center max-w-3xl mx-auto">
          <Box component="span" className="inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-[11px] font-bold uppercase tracking-widest" style={{ background: TINT, color: BLUE_DEEP }}>
            <Rocket className="h-3.5 w-3.5" /> The Biglyp platform
          </Box>
          <Typography variant="inherit" component="h2" className="font-head mt-4 text-3xl md:text-4xl font-black tracking-tight leading-tight" style={{ color: NAVY }}>
            One platform. Three powerful engines.
          </Typography>
          <Typography variant="inherit" component="p" className="mt-4 text-[15px] text-slate-600">
            Everything students, parents and institutions need — unified under Biglyp.
          </Typography>
        </Box>
        ...
        <Box className="mt-12 grid md:grid-cols-3 gap-5">
          {ENGINES.map((e, i) => {
            const Icon = e.icon;
            return (
              <motion.div key={e.title} initial={{ opacity: 0, y: 22 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}
                className="group rounded-3xl border border-blue-100 bg-white p-7 shadow-[0_20px_50px_-32px_rgba(37,99,235,0.45)] hover:-translate-y-1 transition-transform flex flex-col">
                <Box className="flex items-center justify-between">
                  <Box className="h-14 w-14 rounded-2xl flex items-center justify-center" style={{ background: e.color + '16', color: e.color }}><Icon className="h-7 w-7" /></Box>
                  <Box component="span" className="text-[10px] font-bold uppercase tracking-widest rounded-full px-2.5 py-1" style={{ background: TINT, color: BLUE_DEEP }}>{e.tag}</Box>
                </Box>
                <Typography variant="inherit" component="h3" className="font-head text-2xl font-black mt-5" style={{ color: NAVY }}>{e.title}</Typography>
                <Typography variant="inherit" component="p" className="mt-1 text-[13px] font-semibold" style={{ color: e.color }}>{e.tagline}</Typography>
                <Typography variant="inherit" component="p" className="mt-3 text-[13.5px] leading-relaxed text-slate-600">{e.desc}</Typography>
                <Box component="ul" className="mt-5 space-y-2.5 flex-1">
                  {e.points.map((p) => (
                    <Box component="li" key={p} className="flex items-start gap-2 text-[13px] text-slate-700">
                      <Box className="h-4 w-4 rounded-full flex items-center justify-center mt-0.5 shrink-0" style={{ background: e.color + '18', color: e.color }}><Check className="h-3 w-3" /></Box>
                      {p}
                    </Box>
                  ))}
                </Box>
                <Link href={e.href} className="mt-6 inline-flex items-center gap-2 text-sm font-bold" style={{ color: e.color }}>
                  Explore {e.title} <ArrowRight className="h-4 w-4 group-hover:translate-x-1 transition-transform" />
                </Link>
              </motion.div>
            );
          })}
...
const REVIEWS = [
  {
    name: 'Saransh Waghela', role: 'Student', tag: 'DiscoverU', color: SKY,
    text: 'This assessment was very helpful to identify and acknowledge my career goals, and gave insight about my weakness and strengths. I really liked the way everything was so organized and direct. Everything written in the report — about my strengths and weaknesses — was exactly the obstacles I was facing to organize, plan, and simplify my goals.',
  },
  {
    name: 'Divakar Shetty', role: 'Father of Ageithya · Creative Consultant', tag: 'Explored', color: '#7C3AED',
    text: 'As parents, we were a little skeptical before taking the psychometric assessment for our son. But when we received the report (DiscoverU), we were pleasantly surprised by how accurately it described him. Many observations about his interests, learning style, strengths and personality matched what we see at home. It also highlighted a few areas we hadn\u2019t thought about before — a very useful experience that gave us greater confidence in understanding his potential and future direction.',
  },
  {
    name: 'Ritika Menon', role: 'Student', tag: 'Discovery', color: '#EC4899',
    text: 'Biglyp made my study-abroad journey feel simple. From shortlisting the right universities to sorting out my education loan, everything was in one place — with guidance at every single step.',
  },
];
        ...
        <Box className="mt-12 grid md:grid-cols-3 gap-5">
          {REVIEWS.map((r, i) => (
            <motion.div key={r.name} initial={{ opacity: 0, y: 22 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08 }}
              ...
              <Box className="flex gap-0.5 mt-3">
                {[0, 1, 2, 3, 4].map((s) => <Star key={s} className="h-4 w-4" style={{ color: '#F59E0B', fill: '#F59E0B' }} />)}
              </Box>
              ...
              <Box className="mt-5 flex items-center gap-3 pt-4 border-t border-blue-50">
                ...
                <Box>
                  <Typography variant="inherit" component="p" className="font-head font-bold text-[14px]" style={{ color: NAVY }}>{r.name}</Typography>
                  <Typography variant="inherit" component="p" className="text-[11.5px] text-slate-500">{r.role}</Typography>
                </Box>
              </Box>
            </motion.div>
          ))}
  ...
  return (
    <Box component="section" className="py-20 bg-white">
      <Box className="max-w-4xl mx-auto px-5">
        <Box className="text-center">
          <Typography variant="inherit" component="h2" className="font-head text-3xl md:text-4xl font-black tracking-tight" style={{ color: NAVY }}>Frequently asked questions</Typography>
        </Box>
          ...
          {FAQS.map((f, i) => {
            ...
            return (
              <Box key={f.q} className="rounded-2xl border border-blue-100 bg-white overflow-hidden transition-shadow" style={on ? { boxShadow: '0 20px 40px -28px rgba(37,99,235,0.4)' } : undefined}>
                <Box component="button" onClick={() => setOpen(on ? null : i)} data-testid={`faq-${i}`}
      ...
      <Box className="max-w-7xl mx-auto px-5 py-14 grid gap-10 md:grid-cols-[1.4fr_1fr_1fr_1fr]">
        <Box>
          ...
          <Typography variant="inherit" component="p" className="mt-3 text-[13px] text-white/60 leading-relaxed max-w-xs">
            Building future-ready students — from early career discovery to the right course, university and beyond.
          </Typography>
          <Box className="mt-5">
        ...
        <Box>
          ...
          <Box component="ul" className="mt-4 space-y-2 text-[13px] text-white/80">
            {['Study in USA', 'Study in UK', 'Study in Canada', 'Study in Australia', 'Study in Germany', 'Study in India'].map((d) => (
              <Box component="li" key={d}>{d}</Box>
            ))}
          </Box>
        </Box>
        <Box>
...
export default function Homepage() {
  return (
    <Box className="min-h-screen bg-white text-slate-800 font-sans">
      ...
      <TabbedSection
        id="journey"
        eyebrow="Complete education journey"
        ...
        bg={TINT}
        tabs={[
          { label: 'Psychometric Assessment', icon: Brain, cardTitle: 'Biglyp Discovery', img: LEARN_IMG, primary: 'Take Assessment', secondary: 'Learn more',
            desc: ['Take our AI-powered psychometric assessment to uncover your strengths, interests, and ideal career paths. Get personalized recommendations based on 60+ personality traits.', 'Discover careers that match your passions and future goals.'] },
          { label: 'AI Career Search', icon: Search, cardTitle: 'AI Career Search', img: LEARN_IMG, primary: 'Search Careers', secondary: 'Learn more',
            desc: ['Search across 2,50,000+ courses and programs in 42 countries. Our AI matches your profile to the best-fit careers and universities in seconds.'] },
          { label: 'Admission Assistance', icon: FileCheck2, cardTitle: 'Admission Assistance', img: LEARN_IMG, primary: 'Get Started', secondary: 'Learn more',
            desc: ['End-to-end application support — shortlisting, documentation, SOPs and interview prep — so you never miss a deadline or a dream college.'] },
          { label: 'Exam Preparation', icon: BookOpenCheck, cardTitle: 'Exam Preparation', img: LEARN_IMG, primary: 'Start Prep', secondary: 'Learn more',
            desc: ['Structured prep for competitive and entrance exams with expert-curated content, mock tests and real-time progress tracking.'] },
          { label: 'Scholarships', icon: Award, cardTitle: 'Scholarships', img: LEARN_IMG, primary: 'Find Scholarships', secondary: 'Learn more',
            desc: ['Discover and apply to scholarships you actually qualify for, with guided applications and smart deadline reminders.'] },
        ]}
      />
      <TabbedSection
        id="financing"
        eyebrow="Smart financial solutions"
        ...
        tabs={[
          { label: 'Education Financing', icon: Landmark, cardTitle: 'Education Financing', img: LEARN_IMG, primary: 'Know more',
            desc: ['Explore the best education loan options, compare lenders, check eligibility, estimate EMIs, and receive expert guidance to finance your higher education journey with confidence and ease.', 'Compare education loans from trusted financial institutions.'] },
          { label: 'EMI Calculator', icon: Calculator, cardTitle: 'EMI Calculator', img: LEARN_IMG, primary: 'Calculate EMI',
            desc: ['Estimate your monthly EMIs instantly. Adjust loan amount, tenure and interest rate to plan repayments that fit your budget.'] },
          { label: 'Check Your Loan Eligibility', icon: Gauge, cardTitle: 'Check Your Loan Eligibility', img: LEARN_IMG, primary: 'Check Eligibility',
            desc: ['Check how much you can borrow in minutes — a soft check that never impacts your credit score.'] },
          { label: 'Repayment Calculator', icon: LineChart, cardTitle: 'Repayment Calculator', img: LEARN_IMG, primary: 'Plan Repayment',
            desc: ['Visualise your full repayment schedule, interest breakup and moratorium options before you commit.'] },
          { label: 'Check Your CIBIL Score', icon: ShieldCheck, cardTitle: 'Check Your CIBIL Score', img: LEARN_IMG, primary: 'Check CIBIL Score',
            desc: ['Check your CIBIL score for free and get personalised tips to improve it before you apply for education finance.'] },
        ]}
      />
      <PlatformEngines />
      <Testimonials />
