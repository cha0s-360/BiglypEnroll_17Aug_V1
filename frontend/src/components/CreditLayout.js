    ...
    <div className="min-h-screen flex bg-muted/40">
      <aside className="w-64 shrink-0 bg-brand-navy text-white flex-col hidden md:flex sticky top-0 h-screen">
        <div className="p-6 border-b border-white/10">
          <div className="flex items-center gap-2">
            <div className="h-8 w-8 rounded-sm bg-brand-blue flex items-center justify-center">
              <Landmark className="h-4 w-4 text-white" />
            </div>
            <div>
              <p className="font-head font-extrabold leading-none">Biglyp</p>
        ...
        <nav className="flex-1 p-4 space-y-1">
          ...
          {!isLender && (
            <Link to="/dashboard" data-testid="cnav-back"
              className="flex items-center gap-3 px-4 py-3 rounded-sm text-sm font-medium text-white/50 hover:bg-white/10 hover:text-white transition-colors mt-4">
      ...
      <div className="flex-1 min-w-0">
        <header className="min-h-16 bg-white border-b border-border flex items-center justify-between px-6 py-3 sticky top-0 z-10">
          <div>
            <h1 className="font-head text-xl font-bold tracking-tight text-brand-navy" data-testid="credit-page-title">{title}</h1>
            {subtitle && <p className="text-xs text-muted-foreground mt-0.5">{subtitle}</p>}
          </div>
          <span className="text-xs px-3 py-1.5 rounded-sm bg-brand-tint text-brand-navy font-semibold">RBI DLG · DPDP compliant</span>
        </header>
