...
export default function Reminders() {
  ...
  if (!settings) {
    return <DashboardLayout title="Fee Reminders"><div className="flex justify-center py-20"><Loader2 className="h-6 w-6 animate-spin text-brand-blue" /></div></DashboardLayout>;
  }
      ...
      <div className="max-w-3xl space-y-6">
        ...
        <div className="rounded-xl border border-border bg-white p-5 flex items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            <div className={`h-10 w-10 rounded-lg flex items-center justify-center ${settings.enabled ? "bg-brand-blue text-white" : "bg-muted text-muted-foreground"}`}>
              <Bell className="h-5 w-5" />
            </div>
            <div>
              <p className="font-semibold text-brand-navy">Automatic fee reminders</p>
              <p className="text-xs text-muted-foreground mt-1 max-w-md">When on, parents automatically get in-app notifications (and queued emails) before due dates and for overdue fees, based on the rules below.</p>
            </div>
          </div>
        ...
        {/* Before due */}
        <div className={`rounded-xl border border-border bg-white p-5 ${disabled ? "opacity-50 pointer-events-none" : ""}`}>
          <div className="flex items-center gap-2 mb-1">
            <CalendarClock className="h-4 w-4 text-brand-blue" />
            <p className="font-semibold text-brand-navy">Before the due date</p>
          </div>
          <p className="text-xs text-muted-foreground mb-3">Select how many days before the due date to nudge parents. Each selected day sends one reminder.</p>
          <div className="flex flex-wrap gap-2" data-testid="before-days">
            {BEFORE_CHOICES.map((d) => {
              const on = settings.before_due_days.includes(d);
              return (
                <button key={d} onClick={() => toggleDay("before_due_days", d)} data-testid={`before-${d}`}
        ...
        {/* On due */}
        <div className={`rounded-xl border border-border bg-white p-5 flex items-center justify-between ${disabled ? "opacity-50 pointer-events-none" : ""}`}>
          <div className="flex items-center gap-2">
            <Clock className="h-4 w-4 text-brand-blue" />
            <div>
              <p className="font-semibold text-brand-navy">On the due date</p>
              <p className="text-xs text-muted-foreground">Send a reminder on the exact day fees are due.</p>
            </div>
          </div>
        ...
        <div className={`rounded-xl border border-border bg-white p-5 ${disabled ? "opacity-50 pointer-events-none" : ""}`}>
          <div className="flex items-center gap-2 mb-1">
            ...
            <p className="font-semibold text-brand-navy">After the due date (overdue)</p>
          </div>
          <p className="text-xs text-muted-foreground mb-3">Select how many days after the due date to follow up on unpaid fees.</p>
          <div className="flex flex-wrap gap-2" data-testid="overdue-days">
            {OVERDUE_CHOICES.map((d) => {
              const on = settings.overdue_days.includes(d);
              return (
                <button key={d} onClick={() => toggleDay("overdue_days", d)} data-testid={`overdue-${d}`}
