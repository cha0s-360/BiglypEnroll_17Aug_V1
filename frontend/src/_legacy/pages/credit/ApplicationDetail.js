...
function Row({ k, v, strong }) {
  return (
    <div className="flex justify-between py-2 border-b border-border last:border-0 text-sm">
      ...
      <span className={strong ? "font-head font-bold text-brand-navy" : "font-medium text-brand-navy"}>{v}</span>
    </div>
  );
}
...
export default function ApplicationDetail() {
  ...
  if (!app) return <CreditLayout title="Application"><div className="h-64 flex items-center justify-center"><Loader2 className="h-8 w-8 animate-spin text-brand-blue" /></div></CreditLayout>;
    ...
    <CreditLayout title={app.app_no} subtitle={`${app.applicant?.name || ""} · ${app.student?.name || ""}`}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs capitalize text-muted-foreground px-2 py-1 rounded-sm bg-muted">{(app.status || "").replace(/_/g, " ")}</span>
        </div>
      ...
      <Tabs defaultValue="overview">
            ...
            {app.kyc?.status ? (
              <div className="grid grid-cols-2 gap-2 text-sm">
                {[["PAN", app.kyc.pan_verified], ["Aadhaar", app.kyc.aadhaar_verified], ["DigiLocker", app.kyc.digilocker], ["Selfie/Liveness", app.kyc.selfie_liveness], ["CKYC", !!app.kyc.ckyc_ref], ["e-Sign ready", app.kyc.esign_ready]].map(([l, ok]) => (
                  <div key={l} className="flex items-center gap-2 border border-border rounded-sm px-3 py-2">
                ...
                {app.kyc.ckyc_ref && <p className="col-span-2 text-xs text-muted-foreground">CKYC Ref: {app.kyc.ckyc_ref}</p>}
              </div>
            ) : <Empty text="Run assessment to verify KYC (PAN, Aadhaar, CKYC, DigiLocker)." />}
          </Card>
          <Card title="Document management (OCR)" icon={Upload}>
            <input ref={docRef} type="file" hidden accept=".pdf,.png,.jpg,.jpeg" onChange={uploadDoc} data-testid="doc-input" />
            ...
            <div className="space-y-2">
              {(app.documents || []).map((d) => (
                <div key={d.id} className="border border-border rounded-sm p-3 text-sm">
                  <p className="font-medium text-brand-navy">{d.type} · {d.filename}</p>
                  {d.ocr_data && Object.keys(d.ocr_data).length > 0 && (
                    <p className="text-xs text-muted-foreground mt-1">OCR: {Object.entries(d.ocr_data).map(([k, v]) => `${k}: ${v}`).join(" · ")}</p>
                  )}
                </div>
              ))}
            ...
            <div className="grid lg:grid-cols-3 gap-4">
              <Card title={`Bureau — ${app.bureau.provider}`} icon={Gauge}>
                <div className="text-center py-4">
                  <p className="font-head text-5xl font-black" style={{ color: SCORE_COLOR(app.bureau.score) }}>{app.bureau.score}</p>
                  <p className="text-xs text-muted-foreground mt-1">Bureau score (300–900)</p>
                </div>
              ...
              <Card title="Trade lines" icon={FileText}>
                {(app.bureau.accounts || []).length ? app.bureau.accounts.map((a, i) => (
                  <div key={i} className="border-b border-border last:border-0 py-2 text-sm">
                    <div className="flex justify-between"><span className="text-brand-navy font-medium">{a.type}</span><span className="text-muted-foreground">DPD {a.dpd}</span></div>
                    <p className="text-xs text-muted-foreground">O/s {inr(a.outstanding)} · EMI {inr(a.emi)}</p>
                  </div>
                )) : <p className="text-sm text-muted-foreground">No active trade lines.</p>}
              </Card>
            </div>
        ...
        <TabsContent value="bank" className="mt-4 grid lg:grid-cols-2 gap-4">
          <Card title="Bank statement analyzer" icon={Landmark}>
            <input ref={bankRef} type="file" hidden accept=".pdf,.csv,.txt" onChange={uploadBank} data-testid="bank-input" />
        ...
        <TabsContent value="score" className="mt-4 grid lg:grid-cols-2 gap-4">
          <Card title="Biglyp Internal Credit Score" icon={Gauge}>
            {score ? (
              <>
                <div className="text-center py-4">
                  <p className="font-head text-6xl font-black" style={{ color: SCORE_COLOR(score) }}>{score}</p>
                  <p className="text-xs text-muted-foreground">out of 1000 · <span className="font-semibold">{app.internal_score.band}</span></p>
                </div>
                <div className="space-y-2">
                  {Object.entries(app.internal_score.breakdown).map(([k, v]) => (
                    <div key={k}>
                      <div className="flex justify-between text-xs mb-1">
                        <span className="capitalize text-muted-foreground">{k.replace(/_/g, " ")} <span className="text-[10px]">({Math.round((app.internal_score.weights[k] || 0) * 100)}% wt)</span></span>
                        <span className="text-brand-navy font-medium">{v}/100</span>
                      </div>
          ...
          <Card title="FOIR calculator" icon={Percent}>
            {app.foir?.foir_pct !== undefined ? (
              <>
                <div className="text-center py-4">
                  <p className="font-head text-5xl font-black text-brand-navy">{app.foir.foir_pct}%</p>
                  <p className="text-xs text-muted-foreground">Fixed Obligation to Income Ratio</p>
                </div>
            ...
            <>
              <div className="bg-white border border-border rounded-sm p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs tracking-[0.2em] uppercase text-muted-foreground">Eligibility decision</p>
                    <p className="font-head text-2xl font-black text-brand-navy mt-1">{dec.status}</p>
                  </div>
                ...
                {dec.recommended_lender_name && (
                  <div className="mt-4 bg-brand-tint rounded-sm p-4 flex items-center gap-3">
                    <Landmark className="h-5 w-5 text-brand-blue" />
                    <p className="text-sm text-brand-navy">Best lender recommendation: <b>{dec.recommended_lender_name}</b> (highest approval probability)</p>
                  </div>
                )}
              </div>
              <div className="bg-white border border-border rounded-sm overflow-hidden">
                <div className="p-4 border-b border-border bg-muted/30"><h3 className="font-head font-bold text-brand-navy">Lender-by-lender policy match</h3></div>
                <table className="w-full text-sm">
                  <thead><tr className="text-left text-xs uppercase text-muted-foreground border-b border-border">
                    <th className="p-3">Lender</th><th className="p-3">Rate</th><th className="p-3">Result</th><th className="p-3">Approval prob.</th><th className="p-3">Failed rules</th><th className="p-3"></th>
                  </tr></thead>
                  <tbody>
                    {(dec.per_lender || []).map((e) => (
                      <tr key={e.lender_id} className="border-b border-border last:border-0" data-testid={`lender-${e.lender_id}`}>
                        <td className="p-3"><span className="font-medium text-brand-navy">{e.lender_name}</span> <span className="text-xs text-muted-foreground">({e.lender_type})</span></td>
                        <td className="p-3">{e.interest_rate}%</td>
                        <td className="p-3">{e.passed ? <Badge className="bg-green-100 text-green-700 rounded-sm">Pass</Badge> : <Badge className="bg-red-100 text-red-700 rounded-sm">Fail</Badge>}</td>
                        <td className="p-3">
                          <div className="flex items-center gap-2">
                            <div className="w-16 h-2 bg-muted rounded-sm overflow-hidden"><div className="h-full bg-brand-blue" style={{ width: `${e.approval_probability * 100}%` }} /></div>
                            <span className="text-xs">{Math.round(e.approval_probability * 100)}%</span>
                          </div>
                        </td>
                        <td className="p-3 text-xs text-muted-foreground max-w-xs">{e.failed_rules?.length ? e.failed_rules.join("; ") : "—"}</td>
                        <td className="p-3">
                ...
                {app.fraud.flags?.length ? (
                  <ul className="space-y-2">
                    {app.fraud.flags.map((f, i) => (
                      <li key={i} className="flex items-start gap-2 text-sm border border-red-100 bg-red-50 rounded-sm px-3 py-2">
                        <AlertTriangle className="h-4 w-4 text-red-500 mt-0.5" /> <span className="text-red-700">{f}</span>
                      </li>
                    ))}
                  </ul>
                ) : <p className="text-sm text-green-700 flex items-center gap-2"><CheckCircle2 className="h-4 w-4" /> No fraud signals detected (duplicate PAN/mobile, tampering, velocity, statement anomalies).</p>}
              </>
        ...
        <TabsContent value="workflow" className="mt-4 grid lg:grid-cols-2 gap-4">
          <Card title="Maker-Checker workflow" icon={GitBranch}>
            ...
            <div className="flex flex-wrap gap-2 mt-4">
              ...
              {wf.stage === "checker" && !isAdmin && <p className="text-xs text-muted-foreground">Awaiting checker (credit ops / admin) review.</p>}
              {wf.stage === "lender_submission" && <p className="text-xs text-muted-foreground">Approved by checker — submit to a lender from the Decision tab.</p>}
            </div>
          </Card>
          <Card title="Audit trail" icon={ScrollText}>
            <div className="space-y-2 max-h-80 overflow-y-auto">
              {audit.map((e) => (
                <div key={e.id} className="text-xs border-b border-border pb-2">
                  <p className="font-medium text-brand-navy capitalize">{e.action.replace(/_/g, " ")}</p>
                  <p className="text-muted-foreground">{e.actor} · {new Date(e.timestamp).toLocaleString("en-IN")}</p>
                </div>
              ))}
  ...
  return (
    <div className="bg-white border border-border rounded-sm p-6">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="font-head font-bold text-brand-navy">{title}</h3>
      </div>
      {children}
    </div>
  );
