...
export default function Policies() {
  ...
  return (
    <CreditLayout title="Lender Policies & Score Model" subtitle="Configure credit rules without code — changes apply to new assessments">
      ...
      {config && (
        <div className="bg-white border border-border rounded-sm p-6 mb-6">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2"><Gauge className="h-4 w-4 text-brand-blue" /><h3 className="font-head font-bold text-brand-navy">Biglyp Internal Score weightages</h3></div>
      ...
      {/* Lender policies */}
      <div className="flex items-center gap-2 mb-3"><SlidersHorizontal className="h-4 w-4 text-brand-blue" /><h3 className="font-head font-bold text-brand-navy">Lender credit policies</h3></div>
          ...
          <AccordionItem key={l.id} value={l.id} className="bg-white border border-border rounded-sm px-4" data-testid={`policy-${l.id}`}>
            <AccordionTrigger className="hover:no-underline">
              <div className="flex items-center gap-3">
                <div className="h-8 w-8 rounded-sm flex items-center justify-center" style={{ background: l.color }}><Landmark className="h-4 w-4 text-white" /></div>
                <span className="font-medium text-brand-navy">{l.name}</span>
