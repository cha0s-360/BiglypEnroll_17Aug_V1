    ...
    <DashboardLayout title="Fee Structure">
      ...
      <Box className="flex flex-wrap items-center justify-between gap-3 mb-5">
        <Box className="flex items-center gap-2">
          ...
          <Box component="span" className="text-sm text-muted-foreground">{fs.fee_heads.length} fee heads</Box>
        </Box>
        <Box className="flex flex-wrap gap-2">
          <Box component="input" ref={fileRef} type="file" accept=".xlsx,.xls,.csv" hidden onChange={onUpload} data-testid="fee-file-input" />
      ...
      <Box className="bg-white border border-border rounded-sm overflow-hidden">
        <Box className="flex items-center justify-between p-4 border-b border-border bg-muted/30">
          <Typography variant="inherit" component="h3" className="font-head font-bold text-brand-navy">Fee heads</Typography>
        ...
        <Box className="overflow-x-auto">
          <Box component="table" className="w-full text-sm">
            <Box component="thead">
              <Box component="tr" className="text-left text-xs uppercase tracking-wider text-muted-foreground border-b border-border">
                <Box component="th" className="p-4 font-medium">Fee name</Box>
                <Box component="th" className="p-4 font-medium">Amount (₹)</Box>
                <Box component="th" className="p-4 font-medium">Frequency</Box>
                <Box component="th" className="p-4 font-medium">Applicable grades</Box>
                <Box component="th" className="p-4"></Box>
              </Box>
            </Box>
            <Box component="tbody">
              {fs.fee_heads.map((h) => (
                <Box component="tr" key={h.id} className="border-b border-border last:border-0" data-testid={`fee-row-${h.id}`}>
                  ...
                  <Box component="td" className="p-3">
                    <Box className="flex flex-wrap gap-1 max-w-[260px]">
                      {GRADES.map((g) => (
                        <Box component="button" key={g} onClick={() => toggleGrade(h.id, g)}
                  ...
                  <Box component="td" className="p-3">
                    <Box component="button" onClick={() => removeHead(h.id)} className="text-muted-foreground hover:text-destructive" data-testid={`del-head-${h.id}`}>
              ...
              {fs.fee_heads.length === 0 && (
                <Box component="tr"><td colSpan={5} className="p-10 text-center text-muted-foreground">No fee heads yet. Add one or upload an Excel sheet.</Box></Box>
              )}
            </Box>
          </Box>
        </Box>
      ...
      <Box className="mt-4 grid lg:grid-cols-2 gap-4">
        <Box className="bg-white border border-border rounded-sm">
          <Box className="flex items-center justify-between p-4 border-b border-border bg-muted/30">
            <Typography variant="inherit" component="h3" className="font-head font-bold text-brand-navy flex items-center gap-2"><Award className="h-4 w-4" /> Scholarships</Typography>
          ...
          <Box className="p-4 space-y-3">
            {fs.scholarships.map((s) => (
              <Box key={s.id} className="flex items-center gap-2" data-testid={`sch-row-${s.id}`}>
                ...
                <Box component="button" onClick={() => removeSch(s.id)} className="text-muted-foreground hover:text-destructive"><Trash2 className="h-4 w-4" /></Box>
              </Box>
            ))}
            {fs.scholarships.length === 0 && <Typography variant="inherit" component="p" className="text-sm text-muted-foreground py-4 text-center">No scholarships configured.</Typography>}
          </Box>
        </Box>

        <Box className="bg-white border border-border rounded-sm">
          <Box className="p-4 border-b border-border bg-muted/30">
            <Typography variant="inherit" component="h3" className="font-head font-bold text-brand-navy">Discount & late-fee rules</Typography>
          </Box>
          <Box className="p-4 space-y-4">
            ...
            <Typography variant="inherit" component="p" className="text-xs text-muted-foreground">
