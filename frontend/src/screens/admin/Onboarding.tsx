    ...
    <DashboardLayout title="School Setup">
        ...
        {STEPS.map((s, i) => {
          ...
          return (
            <Box key={s.key} className="flex items-center gap-2 shrink-0">
              <Box component="button" onClick={() => (school || i === 0) && setStep(i)}
      ...
      <Box className="max-w-3xl bg-white border border-border rounded-sm p-8">
        <Box className="flex items-center gap-3 mb-6">
          <Box className="h-10 w-10 rounded-sm bg-brand-blue flex items-center justify-center">
        ...
        {step === 3 && (
          <Box className="space-y-5">
            <Box className="flex items-center justify-between border border-border rounded-sm p-4">
              <Box>
                <Typography variant="inherit" component="p" className="font-medium text-brand-navy">Multi-account settlement</Typography>
                <Typography variant="inherit" component="p" className="text-xs text-muted-foreground">Route different fee heads to different bank accounts.</Typography>
              </Box>
        ...
        {step === 4 && (
          <Box className="space-y-5">
            <Typography variant="inherit" component="p" className="text-sm text-muted-foreground -mt-2">
            ...
            {[
              ...
              return (
                <Box key={o.key} className="flex items-start justify-between gap-4 border border-border rounded-sm p-4" data-testid={`payopt-${o.key}`}>
                  <Box className="flex items-start gap-3">
                    <Box className={`h-10 w-10 rounded-sm flex items-center justify-center shrink-0 ${on ? "bg-brand-blue text-white" : "bg-muted text-muted-foreground"}`}>
                      <Icon className="h-5 w-5" />
                    </Box>
                    <Box>
                      <Typography variant="inherit" component="p" className="text-[10px] uppercase tracking-widest font-bold text-brand-blue">{o.tag}</Typography>
                      <Typography variant="inherit" component="p" className="font-medium text-brand-navy leading-tight">{o.title}</Typography>
                      <Typography variant="inherit" component="p" className="text-xs text-muted-foreground mt-1 max-w-md">{o.desc}</Typography>
                    </Box>
                  </Box>
  ...
  return (
    <Box className="space-y-4">
      <Box className="space-y-3">
        {items.map((it) => (
          <Box key={it.id} className="flex items-center gap-2" data-testid={`${testid}-row`}>
            {fields.map((f) => (
              <Input key={f.k} value={it[f.k]} placeholder={f.ph}
                onChange={(e) => edit(it.id, f.k, e.target.value)} className="rounded-sm h-10" />
            ))}
            <Box component="button" onClick={() => remove(it.id)} className="text-muted-foreground hover:text-destructive shrink-0">
              <Trash2 className="h-4 w-4" />
            </Box>
          </Box>
        ))}
        {items.length === 0 && <Typography variant="inherit" component="p" className="text-sm text-muted-foreground py-3">Nothing added yet.</Typography>}
      </Box>
...
function AccountsEditor({ accounts, setAccounts, feeHeads }) {
  ...
  return (
    <Box className="space-y-4">
      <Box className="space-y-4">
        {accounts.map((a) => (
          <Box key={a.id} className="border border-border rounded-sm p-4 space-y-3" data-testid="account-row">
            <Box className="flex items-center justify-between">
              <Typography variant="inherit" component="p" className="text-sm font-semibold text-brand-navy flex items-center gap-2"><Landmark className="h-4 w-4 text-brand-blue" /> Settlement account</Typography>
              <Box component="button" onClick={() => remove(a.id)} className="text-muted-foreground hover:text-destructive"><Trash2 className="h-4 w-4" /></Box>
            </Box>
            ...
            <Box className="flex items-end gap-3">
              <Box className="flex-1">
                <Label className="text-xs">Account holder name {a.account_name && <Box component="span" className="text-green-600 ml-1">(auto-fetched)</Box>}</Label>
            ...
            <Box>
              ...
              <Select value={a.fee_head_id || ""} onValueChange={(v) => edit(a.id, { fee_head_id: v })}>
                ...
                <SelectContent>
                  {feeHeads.length === 0 && <Box className="px-3 py-2 text-xs text-muted-foreground">Add fee heads first</Box>}
                  {feeHeads.map((h) => (
        ...
        {accounts.length === 0 && <Typography variant="inherit" component="p" className="text-sm text-muted-foreground py-3">No settlement accounts yet. Add one to route a fee header to its own bank account.</Typography>}
      </Box>
