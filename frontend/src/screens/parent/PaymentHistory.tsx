  ...
  return (
    <ParentLayout>
      <Box className="flex flex-wrap items-center justify-between gap-4 mb-6">
        <Box>
          <Typography variant="inherit" component="p" className="text-xs tracking-[0.2em] uppercase text-[#5548D1] font-semibold">Payment History</Typography>
          <Typography variant="inherit" component="h1" className="font-head text-3xl font-black tracking-tight text-brand-navy mt-1">Transactions &amp; Schedule</Typography>
        </Box>
      ...
      {planPayments.length > 0 && (
        <Box className="mb-6 rounded-2xl bg-[#EEF0FF] border border-[#5548D1]/15 p-5 flex items-center gap-4" data-testid="upcoming-summary">
          <Box className="h-11 w-11 rounded-xl bg-[#5548D1] text-white flex items-center justify-center shrink-0">
            <CalendarClock className="h-5 w-5" />
          </Box>
          <Box className="flex-1">
            <Typography variant="inherit" component="p" className="text-sm font-semibold text-brand-navy">Upcoming scheduled dues</Typography>
            <Typography variant="inherit" component="p" className="text-xs text-slate-500 mt-0.5">Across your EMI &amp; auto-debit plans. Reminders sent before each debit.</Typography>
          </Box>
          <Box component="span" className="font-head text-2xl font-black text-[#5548D1]">{inr(totalUpcoming)}</Box>
        </Box>
      )}
      ...
      <Box className="space-y-3">
        {payments.map((p) => {
          ...
          return (
            <Box key={p.id} className="bg-white border border-border rounded-2xl overflow-hidden" data-testid={`payment-${p.id}`}>
              <Box className="flex items-center gap-4 p-4">
                <Box className={`h-10 w-10 rounded-lg flex items-center justify-center ${p.financing ? "bg-brand-navy" : p.auto_debit ? "bg-[#5548D1]" : "bg-[#EEF0FF]"}`}>
                ...
                <Box className="flex-1 min-w-0">
                  <Typography variant="inherit" component="p" className="font-medium text-brand-navy text-sm truncate">
                    {p.items?.map((i) => i.name).join(", ")}
                  </Typography>
                  <Typography variant="inherit" component="p" className="text-xs text-slate-500">{p.receipt_no} · {fmtDate(p.created_at)}</Typography>
                </Box>
                <Box className="text-right">
                  <Typography variant="inherit" component="p" className="font-head font-bold text-brand-navy">{inr(p.amount)}</Typography>
                ...
                ) : <Box component="span" className="w-9" />}
              </Box>

              {/* EMI / installment schedule */}
              {hasSchedule && isOpen && (
                <Box className="border-t border-border bg-[#F8FAFC] px-4 py-3" data-testid={`schedule-${p.id}`}>
                  <Typography variant="inherit" component="p" className="text-[11px] uppercase tracking-[0.15em] text-slate-500 font-semibold mb-2">
                    {p.plan_type === "EMI" ? "Monthly EMI Schedule" : "Installment Schedule"}
                  </Typography>
                  <Box className="rounded-xl border border-border divide-y divide-border bg-white">
                    {p.schedule.map((s) => {
                      const paid = s.status === "paid";
                      return (
                        <Box key={s.month} className="flex items-center gap-3 px-4 py-2.5 text-sm">
                          <Box component="span" className={`h-6 w-6 rounded-full flex items-center justify-center shrink-0 ${paid ? "bg-green-100 text-green-600" : "bg-slate-100 text-slate-400"}`}>
                            {paid ? <CheckCircle2 className="h-4 w-4" /> : <Clock className="h-4 w-4" />}
                          </Box>
                          <Box component="span" className="w-14 text-xs font-semibold text-brand-navy">{s.label || `#${s.month}`}</Box>
                          <Box component="span" className="flex-1 flex items-center gap-1.5 text-slate-500 text-xs">
        ...
        {payments.length === 0 && (
          <Box className="bg-white border border-border rounded-2xl p-12 text-center">
            ...
            <Typography variant="inherit" component="p" className="mt-3 text-slate-500 text-sm">No transactions yet.</Typography>
          </Box>
        )}
      </Box>
    </ParentLayout>
